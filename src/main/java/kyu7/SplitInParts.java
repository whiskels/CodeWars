 ...
 * Details:
 *
 * The aim of this kata is to split a given string into different strings of equal size (note size of strings is passed
 * to the method)
 */
...
public class SplitInParts {
    public static String splitInParts(String s, int partLength) {
        ...
        if (s.length() % partLength != 0) {
            sb.append(" ");
            sb.append(s.substring(s.length() - s.length()%partLength));
        }
