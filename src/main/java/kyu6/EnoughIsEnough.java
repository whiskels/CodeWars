...
public class EnoughIsEnough {
    public static int[] deleteNth(int[] elements, int maxOccurrences) {
        if (elements.length == 0 || maxOccurrences == 0) return new int[0];

        List<Integer> elemList = new ArrayList<>(elements.length);
        for (int e : elements)
        {
            elemList.add(e);
        }

        ArrayList<Integer> filteredElements = new ArrayList<>();

        for (int e : elements) {
            if (filteredElements.contains(e)) {
                int currentCount = Collections.frequency(filteredElements,e);
