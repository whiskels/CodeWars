 ...
 * Details:
 *
 * our task in order to complete this Kata is to write a function which formats a duration, given as a number of
...
public class TimeFormatter {
    ...
    public static String formatDuration(int seconds) {
        ...
        for (int i = 0; i < time.size(); i++) {
            TimeUnit unit = time.get(i);
                sb.append(unit.value)
                        .append(" ")
                        .append(unit.name);
                if (unit.value > 1) sb.append("s");
                if (i < time.size() - 2) sb.append(", ");
                if (i == time.size()- 2) sb.append(" and ");
        }
