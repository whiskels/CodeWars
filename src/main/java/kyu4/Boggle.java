...
public class Boggle {
    ...
    public boolean check() {
        ...
        for (int y = 0; y < board.length; y++) {
            for (int x = 0; x < board[y].length; x++) {
                if (board[y][x] == word.charAt(0) && findMatches(x, y, 1, usedCoords))
                    return true;
            }
        }
        return false;
    ...
    private boolean findMatches(int prevX, int prevY, int currentIndex, int[][] usedCoords) {
            ...
            for (int x = prevX - 1; x <= prevX + 1; x++) {
                if (y >= 0 && x >= 0 && y < board.length && x < board[y].length) {
                    if (board[y][x] == currentChar && (usedCoords[y][x] != 1)) {
                        if (findMatches(x, y, currentIndex + 1, usedCoords))
                            return true;
                    }
                }
            }
