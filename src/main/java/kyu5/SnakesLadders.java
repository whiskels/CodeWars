 ...
 *  independently of the state of the game or the player turn. The variables die1 and die2 are the die thrown in a turn
 ...
 * exactly on a square that shows an image of the bottom of a ladder, then you may move the player all the way up to the
 * square at the top of the ladder. (even if you roll a double).
 *      6.  Slide down snakes. Snakes move you back on the board because you have to slide down them. If you land
 * exactly at the top of a snake, slide move the player all the way to the square at the bottom of the snake or chute.
...
public class SnakesLadders {
    ...
    public String currentState(Player p) {
    ...
    public void checkPosition(Player p) {
        modifiers.forEach(m -> {if (m.startX == p.x) p.x = m.endX;});
    }
        ...
        for (int i = 0; i < players.size(); i++) {
            if (players.get(i).isCurrentTurn) {
                ...
                if (die1 != die2) {
                    ...
                    else players.get(0).isCurrentTurn = true;
                }
